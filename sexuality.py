...
https://dreams.ucsc.edu/Coding/sexuality.html
"""
...
# Return:
# 	True if the word is a adjective;
#	False otherwise.
##########################

def isAdjective(item,tags):
	if not isinstance(item, list):
		tag = pos_tag([item])[0]
		for t in tags:
			if t[0] == tag[0]:
...
def getNodes(items):
	res =[]
	for node in items:
		if node[1] == 'NN' or node[1] == 'CC' or node[1] == 'PRP' or node[1] == 'POS' or node[1] == 'NNS' or node[1] == 'JJ' or node[1] == 'VBN' or node[1] == 'VB' or node[1] == 'VBD' or node[1] == 'VBG' or node[1] == 'VBN' or node[1] == 'VBP' or node[1] == 'VBZ' or node[1] == 'MD' or node[1] == 'TO' or node[1] == 'DT' or node[1] == 'NNPS' or node[1] == 'RP' or node[1] == 'RB' or node[1] == 'PRP$':
			if node[0] != 'it':
				res.append(node[0])
		elif  node[1] == 'NNP':
...
##########################
# Check if verb 'feel' is in items.
# Parameter:
# 	@items: list of words. 
# Return:
# 	True if verb 'feel' is in items;
#	False otherwise.
##########################

def checkFeel(items):
	for x in items:
		if lemmatizer.lemmatize(x, 'v') == 'feel':
			return True
	return False

##########################
# Check if verb 'make' is in items.
# Parameter:
# 	@items: list of words. 
# Return:
# 	True if verb 'feel' is in items;
...
def checkMake(items):
	for x in items:
		if lemmatizer.lemmatize(x, 'v') == 'make':
			return True
	return False

##########################
# Check if verb 'make' is in items and change it with '-.-' to be able to identify it later.
# Parameter:
# 	@items: list of words. 
# Return:
# 	List of the same words without 'make' verb.
##########################

def changeMake(items):
	new = []
	for x in items:
		if lemmatizer.lemmatize(x, 'v') != 'make':
			new.append(x)
...
##########################
# Extraction and encoding of sexuality interactions within dream reports 
# Parameter:
# 	@text: dream report.
# Return:
# 	list of clean coded sexuality interaction.
##########################

def sexuality_code(text, stanford_parser, nlp):
	...
	for s in senteces:
		...
		for i in range(len(subject)):
			...
			if aggcode != '' and ('S' in aggcode)  and (isVerb(subject[i],items) or isAdjective(subject[i],items) or (isNN(subject[i],items))) and ((not isVerb(subject[i],pos_tag(('I like '+subject[i]).split())) and subject[i] != 'orgasm' or isVerb(subject[i],items)) or ((lemmatizer.lemmatize(subject[i], 'v') == 'love' and checkMake(subject))) or ((subject[i] == 'orgasm' and checkFeel(subject)))):
				if pos_tag(subject[i])[1] == 'VBD' or pos_tag(subject[i])[1] == 'VBN':
					flag = True
				if lemmatizer.lemmatize(subject[i], 'v') == 'love':
					subject = changeMake(subject)
				left = subject[:i][::-1]
				right = subject[i:]
